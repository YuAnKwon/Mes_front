...
interface ShipmentInvoiceProps {
  open: boolean; // ✅ 모달 on/off
  onClose: () => void; // ✅ 닫기 콜백
  id: number | null; // ✅ 출하 ID (props로 받기)
}
...
export default function ShipmentInvoice({
  ...
  return (
    <Modal open={open} onClose={onClose}>
      ...
      >
        {shipData.length === 0 ? (
          <Typography sx={{ p: 2 }}>출하증 정보를 불러오는 중...</Typography>
        ) : (
          <>
            <Box
              id="print-area"
          ...
          >  */}
                  ...
                  <Table size="small" sx={{ borderCollapse: "collapse" }}>
                    <TableBody>
                      <TableRow>
                        ...
                        <TableCell sx={{ ...cellStyle, width: "12%" }}>
                          TEL
                        </TableCell>
            ...
            <Box sx={{ mt: 2, textAlign: "right" }}>
              <Button
                variant="outlined"
                size="large"
                sx={{ px: 4, py: 1, fontSize: "1rem", mr: 1 }}
                onClick={handlePrint}
              >
                출력
              </Button>
